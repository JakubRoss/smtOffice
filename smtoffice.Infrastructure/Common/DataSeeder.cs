    ...
    public class DataSeeder(ISqlConnectionFactory sqlConnectionFactory, IPasswordHasher passwordHasher)
    {
        ...
        public void Seed()
        {
            ...
            if (connection != null)
            {
                ...
                var commandPositions = new SqliteCommand(queryCheckPositions, connection);
                var commandAdmins = new SqliteCommand(queryCheckAdmnins, connection);
                var commandAbsenceReason = new SqliteCommand(queryCheckAbsenceReason, connection);
                var commandSubdivision = new SqliteCommand(queryCheckSubdivision, connection);

                connection.Open();

                // SQLite używa dynamicznych typów i COUNT(*) zwraca Int64,
                // co może powodować InvalidCastException przy rzutowaniu na int.
                // SQL Server zazwyczaj zwraca Int32, ale też może zwrócić bigint.
                var positionCount = Convert.ToInt32(commandSubdivision.ExecuteScalar());
                var adminCount = Convert.ToInt32(commandSubdivision.ExecuteScalar());
                var AbsenceReasonCount = Convert.ToInt32(commandSubdivision.ExecuteScalar());
                var SubdivisionCount = Convert.ToInt32(commandSubdivision.ExecuteScalar());
                ...
                if (adminCount == 0)
                {
                    ...
                    var commandAdminInsert = new SqliteCommand(insertEmployeeQuery, connection);
                    ...
                    var commandAdminUpdate = new SqliteCommand(updateEmployeeQuery, connection);
