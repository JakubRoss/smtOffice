namespace smtOffice.Presentation.Controllers
{
        ...
        IPasswordHasher passwordHasher) : Controller
    {
        ...
        public async Task<IActionResult> Index(LoginDTO model)
        {
            ...
            // Uwaga: użycie `.Result` na metodzie async może spowodować wyjątek typu AggregateException,
            // który ukrywa prawdziwy błąd (np. NullReferenceException). Powoduje też ryzyko deadlocka.
            // Zamiast tego używaj `await`, aby zachować pełną obsługę wyjątków i zgodność z async.
            if (await _accountService.IsValidUser(model))
            {
